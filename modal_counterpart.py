class HashMapper:
    ...
    def _custom_hash(self, word):
        """
        Custom hash function to replace SHA-256.
        Uses polynomial rolling hash with multiple prime multipliers for better distribution.
        """
        # Convert word to bytes
        word_bytes = word.encode('utf-8')
        
        # Multiple hash computations for better distribution
        hash1 = 0
        hash2 = 0
        
        # First hash with prime 31
        for i, byte in enumerate(word_bytes):
            hash1 = (hash1 * 31 + byte) % (2**32)
        
        # Second hash with prime 37, using position weighting
        for i, byte in enumerate(word_bytes):
            hash2 = (hash2 * 37 + byte * (i + 1)) % (2**32)
        
        # Combine hashes using XOR and additional mixing
        combined = hash1 ^ hash2
        combined = combined ^ (combined >> 16)
        combined = combined * 0x45d9f3b
        combined = combined % (2**32)
        combined = combined ^ (combined >> 16)
        
        return combined

    def _compute_hash(self, word):
        """Compute initial hash index for the word using custom hash function."""
        hash_value = self._custom_hash(word)
        return hash_value % self.table_size

    def _compute_step(self, word):
        """Compute a probing step derived from the custom hash (for double hashing-like behavior)."""
        hash_value = self._custom_hash(word)
        # Derive step from higher bits; ensure it's between 1 and table_size-1
        step = 1 + (hash_value // self.table_size) % (self.table_size - 1)
        return step
